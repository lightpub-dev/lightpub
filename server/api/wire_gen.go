...
func initializeTimelineService(c echo.Context, h *Handler) timeline.TimelineService {
	...
	dbUserCreateService := users.ProvideDBUserCreateService(dbConn)
	pubUserService := users.ProvidePubUserService(dbUserCreateService, goRequesterService)
	dbUserFinderService := users.ProvideDBUserFinder(dbConn, pubUserService)
...
var (
	DBSet = wire.NewSet(
		ProvideDBConnFromHandler, db.ProvideContext, ProvideIDGetter, wire.Bind(new(pub.IDGetterService), new(*IDGetter)), ProvideGoRequesterService, wire.Bind(new(pub.RequesterService), new(*pub.GoRequesterService)),
	)
)
